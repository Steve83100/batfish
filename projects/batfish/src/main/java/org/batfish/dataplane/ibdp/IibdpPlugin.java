...
/**
 * A new batfish plugin that registers the Incremental IBDP (iibdp) Engine.
 * This new engine supports an additional feature, which allows recomputation after config changes,
 * enabling simulation of how real-life config changes affect the already-converged network.
 * It's why we call it "incremental" IBDP. It incrementally computes a new converge state based on the old one.
 *
 * For now, the new feature is implemented in a separate engine than the original ibdp so that it remains untouched.
 * Maybe in the future, it can be merged into the original ibdp.
 *
 * Yes, there are two 'i's in the name "iibdp". I know that the 'i' in ibdp already stands for incremental,
 * but I was thinking, maybe this additional 'i' would make it sound...well...more incremental? dunno
 */
...
public final class IibdpPlugin extends DataPlanePlugin {
    ...
    public ComputeDataPlaneResult computeDataPlane(NetworkSnapshot snapshot) {
        ...
        return computeDataPlaneWithIntfMods(snapshot, new LinkedList<InterfaceModification>()); // Compute DP with no changes
    }

    /**
     * This method is not declared in DataPlanePlugin.java, therefore you need to downcast before calling it.
     */
    public ComputeDataPlaneResult computeDataPlaneWithIntfMods(
        ...
        Set<BgpAdvertisement> externalAdverts = _batfish.loadExternalBgpAnnouncements(snapshot, configurations);
        ...
        ComputeDataPlaneResult answer =
                _engine.incrementalComputeDataPlane(
                        ...
                        intfMods);
        _logger.infof(
                "Incrementally generated data-plane for snapshot:%s",
                snapshot.getSnapshot());
        return answer;
    }

    public Map<String, ComputeDataPlaneResult> computeDataPlaneWithLinkPerms(NetworkSnapshot snapshot) {
        LOGGER.info("Iibdp Plugin: Loading information from snapshot...");
        System.out.println("Iibdp Plugin: Loading information from snapshot...");
        Map<String, Configuration> configurations = _batfish.loadConfigurations(snapshot);
        Set<BgpAdvertisement> externalAdverts = _batfish.loadExternalBgpAnnouncements(snapshot, configurations);
        TopologyProvider topologyProvider = _batfish.getTopologyProvider();
...
//        }
        ...
        Collection<Queue<InterfaceModification>> intfPerms = Collections2.transform(
        ...
        System.out.println("Iibdp Plugin: Preparation finished. Starting full data plane computation...");
        System.out.println("---------------------------------------------------------------------------");
...
//            System.out.println("Iibdp Plugin: Generated data plane with hash: " + answer.getHash());
    ...
    public Map<String, ComputeDataPlaneResult> computeDataPlaneWithLinkResets(NetworkSnapshot snapshot) {
        LOGGER.info("Iibdp Plugin: Loading information from snapshot...");
        System.out.println("Iibdp Plugin: Loading information from snapshot...");
        Map<String, Configuration> configurations = _batfish.loadConfigurations(snapshot);
        Set<BgpAdvertisement> externalAdverts = _batfish.loadExternalBgpAnnouncements(snapshot, configurations);
